...
"rk30sdk"|"rk3066"|"rk3188")
	...
	echo -e "\033[01;32m"
	echo "  The first one is for normal users. Detail steps:"
	...
	echo -e "\033[00m"
	echo -e "\033[01;33m"
	echo "  The second one is for real geeks. It ONLY works for device with EMMC flash."
	echo "  You will brick you device if it's mounted with NAND flash."
	echo ""
	echo "  Detail steps:"
	echo "   1. copy \"rockdev\" to windows (or map a network drive from linux to windows)"
	echo "   2. double click \"rockdev\\AndroidTool.exe\" to run the flash application"
	echo "   2a. change the Loader file to RK30xxLoader(L)_V0.02_PRELIMINARY_EMMC_ONLY.bin"
	echo "   3. press recovery key of the device and plug in the USB cable"
	echo "   3a. wait for the Rockusb interface enumerates in windows"
	echo "       you need install driver under \"rockdev\\Driver\" for the 1st time"
	echo "   4. click Run button in AndroidTool to flash"
	echo ""
	echo "  From now on you do not need copy images around, since you can flash with"
	echo "  fastboot under Linux.  To enter fastboot, reboot with argument fastboot."
	echo "  For the partition name, check rockdev/parameter."
	echo ""
